...
// EventsStreamer implementation
EventsStreamer::EventsStreamer()
    : event_queue_(1000) {  // Max 1000 events in queue
}
...
void EventsStreamer::SetReplicaConnection(std::shared_ptr<Connection> connection) {
    connection_ = connection;
}

void EventsStreamer::SetDatabase(const std::string& database) {
    database_ = database;
}

void EventsStreamer::SetTable(const std::string& table) {
    table_ = table;
}

void EventsStreamer::SetChangelogTable(const std::string& changelog_table) {
    changelog_table_ = changelog_table;
}

void EventsStreamer::Initiate() {
    // Get current binlog coordinates from replica
    if (connection_ && connection_->IsConnected()) {
        current_coordinates_ = BinlogCoordinates(
            connection_->GetCurrentBinlogFile(),
            connection_->GetCurrentBinlogPosition()
        );
        initial_coordinates_ = current_coordinates_;
        
        LOG_INFO_FMT("Initial binlog coordinates: {}", current_coordinates_.ToString());
    }
}

BinlogCoordinates EventsStreamer::GetCurrentCoordinates() const {
    return current_coordinates_;
}

bool EventsStreamer::Start() {
    reader_ = std::make_unique<GoMySQLBinlogReader>();
    
    reader_->SetReplicaConnection(connection_);
    reader_->SetDatabase(database_);
    reader_->SetTable(table_);
    reader_->SetStartingCoordinates(initial_coordinates_);
    reader_->SetEventCallback([this](std::shared_ptr<BinlogEntry> entry) {
        HandleEvent(entry);
    });
...
uint64_t EventsStreamer::TotalEventsProcessed() const {
    return reader_ ? reader_->EventsProcessed() : 0;
}

uint64_t EventsStreamer::DMLEventsForTable() const {
    return reader_ ? reader_->DMLEventsProcessed() : 0;
}

void EventsStreamer::HandleEvent(std::shared_ptr<BinlogEntry> entry) {
    if (!ShouldProcessEvent(entry)) {
        return;
    }
    
    // Update current coordinates
    current_coordinates_ = entry->Coordinates();
...
bool EventsStreamer::ShouldProcessEvent(std::shared_ptr<BinlogEntry> entry) const {
    ...
    // Process changelog events for heartbeat/hints
    if (entry->Table() == changelog_table_) {
        return true;
    }
    
    // Only process DML for our target table
    if (!entry->IsDML()) {
        return entry->EventType() == BinlogEventType::Rotate ||
               entry->EventType() == BinlogEventType::Query;
    }
    
    return entry->IsForTable(database_, table_);
}
